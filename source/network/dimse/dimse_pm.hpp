...
class dimse_pm
{
   ...
   private:
       ...
       * @param[in] d data
       */
      void data_handler(upperlayer::Iupperlayer_comm_ops* sc, std::unique_ptr<upperlayer::property> da)
      {
          using namespace upperlayer;
          using namespace dicom::util::log;
          ...
          p_data_tf* dtf = dynamic_cast<p_data_tf*>(da.get());
          assert(da != nullptr); // d == nullptr would imply that this function is bound
              // to the wrong message type.

          commandset_processor proc {dict};
          commandset_data b = proc.deserialize(dtf->command_set);

          BOOST_LOG_SEV(logger, info) << "Message from peer received";

          iod dataset;
          if (!dtf->data_set.empty()) {
              auto& tfproc = find_transfer_processor(dtf->pres_context_id);

              dataset = tfproc.deserialize(dtf->data_set);
